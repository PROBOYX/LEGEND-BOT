...
javes = javes05 = register
WAFU_CHATID=int(os.environ.get("PRIVATE_GROUP_ID", -999))
...
Add them to your harem by sending /protecc character name"""
    ...
    if omk == True:
        ...
        await event.edit(f"`{NAME}`: **auto wafu already present**")
...
async def del_wafu(event):
    try:
    if omk == True:
        await event.edit(f"`{NAME}`: **auto wafu stops**")
    else:
